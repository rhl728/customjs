...
async function getRes(email) {
  ...
  if (data) {
    ...
    document
      .querySelector(".resend-verify-message")
      .classList.add(data.success ? "text-success" : "text-error");
  } else {
    document.querySelector(".resend-verify-message").innerHTML =
      "Failed to send link.";
    document
...
document.addEventListener("DOMContentLoaded", (event) => {
  ...
  var userData = localStorage.getItem(
    "user_data_9ec6ba5290735b1924916dc0912a24ef",
  );
  ...
  if (EMPUSER) {
    ...
    function showPopupMessage(message) {
      if (showPopupFlag) {
        ...
        popup.innerHTML =
          message +
          '<span class="close-btn"><svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="16" height="16" viewBox="0 0 50 50"><path d="M 7.71875 6.28125 L 6.28125 7.71875 L 23.5625 25 L 6.28125 42.28125 L 7.71875 43.71875 L 25 26.4375 L 42.28125 43.71875 L 43.71875 42.28125 L 26.4375 25 L 43.71875 7.71875 L 42.28125 6.28125 L 25 23.5625 Z"></path></svg></span>';
        document.body.appendChild(popup);

        popup.style.cssText =
          "position: fixed; margin: 0 auto; top: 5%; left: 50%; transform: translateX(-50%); background: #fff; color: #000; font-family: Open Sans, sans-serif; padding: 20px; width: auto; border-radius: 5px; box-shadow: 0px 0px 10px 0px #930000; z-index: 9999; text-align: center; transition: Bounce;";
    ...
    showPopupMessage(
      "You are now in the special staff rate section of meathouse.lk. Enjoy your purchase!",
    );
  }
  ...
  window.addEventListener("load", function () {
    ...
    if (window.location.pathname === "/") {
      ...
      document
        ...
        .addEventListener("click", () => x.remove());
      document
        .querySelector(".meathouse-deals-chi img")
        .addEventListener("click", () => x.remove());
    }
...
function processExpireDateElements() {
  ...
  const expireDateElements = document.querySelectorAll(
    '[data-code="expireDate"]',
  );
  ...
  expireDateElements.forEach((expireDateElement) => {
    ...
    const expireDateElementValue =
      expireDateElement.querySelector(".expire-date-value");
...
function recipesFeaturedFetch() {
  const dataURL =
    "https://www.keellsfoods.lk/dcb/dynamic.php?requestType=json&dcbName=recipeListing";
    ...
    .then((data) => {
      ...
      items.forEach((item) => {
        if (item) {
          ...
          listitem.className =
            'card relative grid-mode rounded-md box-border overflow-hidden flex flex-nowrap rounded-md items-center bg-transparent border border-gray-100 transition duration-200 ease-in-out transform hover:-translate-y-1 hover:shadow-md"';
...
document.addEventListener("oms_getTemplateListSuccess", () => {
  ...
  setTimeout(() => {
    console.log("dddd");
    let categoryProductData = document.querySelectorAll(
      ".category-product-data",
    );
    if (categoryProductData) {
      categoryProductData.forEach((categoryProductDataItem) => {
        let data = categoryProductDataItem.dataset.productdata
          ? JSON.parse(categoryProductDataItem.dataset.productdata)
          : false;
        let categoryId = categoryProductDataItem.dataset.categories; //? JSON.parse(categoryProductData.dataset.categories):null;
        // console.log("Category Product Data:", data, categoryId);
        let uniqueVmBtn = document.querySelector(
          `.unique-vm-btn-${categoryId}`,
        );
        if (data) {
          if (uniqueVmBtn) {
              uniqueVmBtn.classList.remove("hidden");
          }
        }
      
      });
    } else {
      console.warn("category-product-data element not found");
    }
  }, 2000);
});